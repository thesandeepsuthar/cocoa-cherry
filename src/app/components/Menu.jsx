...
const defaultFlavors = [
  {
    ...
    description:
      "Dense dark chocolate sponge layered with 54% dark chocolate ganache.",
    imageData:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCBB5DmZ47X-r_cjC6MnSDpnzDpXZ0j6Cv5WHu6PmaQPLFD0-mRfPk8BYoUkWIMcCYkVQHeeglMPzInXvXBPcu53gQywC2Dug5T8lQ81BdhsT5bKA1Dt9LuEkHYDwn_bE3WclatUMSmFOtYuHhmSDyAFJDrD4NlDeIIBgv9J0WHAX8n-hP7BA7DZ2_r3dH1k9YjE5yowrKOMiMie_cc8iqorATKRYFXVP5eQZrr9BawXFKZhyh1MXaOlGc8L0JnBOJyaAa5FCVaccbe",
    badge: "Best Seller",
    price: 850,
    discountPrice: null,
    priceUnit: "per kg",
...
const getCategoryFromName = (name) => {
  ...
  if (nameLower.includes("cookie") || nameLower.includes("khatai"))
    return "Cookies";
  if (
  ...
  if (
    ...
    nameLower.includes("ragi") ||
    nameLower.includes("jaggery") ||
    nameLower.includes("multi grain")
  )
    return "Healthy";
  return "Others";
};
...
function QuickViewModal({
  ...
  const itemsToNavigate =
    categoryItems && categoryItems.length > 0 ? categoryItems : allItems;
      ...
      >
        ...
        <div className="p-4 sm:p-5 md:p-6">
          <div className="flex items-start justify-between gap-3 mb-2 sm:mb-3">
            <h3
              className="text-lg sm:text-xl md:text-2xl font-bold text-cream line-clamp-2"
            ...
            <span className="flex-shrink-0 px-2 py-1 rounded-lg bg-rose/10 text-rose text-[10px] sm:text-xs font-medium border border-rose/20 whitespace-nowrap">
              {(item.category
                ? typeof item.category === "string"
                  ? item.category
                  : item.category.name
                : null) || getCategoryFromName(item.name)}
            </span>
          </div>
          ...
          {item.price > 0 && (
            <div className="flex items-center gap-2 sm:gap-3 mb-5 sm:mb-6 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-noir/50 border border-cream/5">
              <span className="material-symbols-outlined text-gold text-xl sm:text-2xl flex-shrink-0">
                payments
              </span>
              <div className="min-w-0">
                {hasDiscount ? (
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span className="text-lg sm:text-2xl font-bold text-rose">
                      ₹{item.discountPrice}
                    </span>
                    <span className="text-cream/40 line-through text-xs sm:text-sm">
                      ₹{item.price}
                    </span>
                  </div>
                ) : (
                  <span className="text-lg sm:text-2xl font-bold text-cream">
                    ₹{item.price}
                  </span>
                )}
                {item.priceUnit && (
                  <span className="text-cream/50 text-xs sm:text-sm ml-1">
          ...
          <div className="mb-4 sm:mb-5 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-noir/50 border border-cream/5">
            ...
            {item.price > 0 && (
              <p className="text-xs sm:text-sm text-cream/60 mt-2 text-center">
                Estimated: ₹
                {((item.discountPrice || item.price) * quantity).toFixed(0)}
              </p>
            )}
          </div>
                  ...
                  <div className="flex items-center gap-2 pl-14">
                    <button
                      onClick={() =>
                        onUpdateQuantity(
                          item._id,
                          Math.max(0.5, (item.quantity || 1) - 0.5),
                        )
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        onUpdateQuantityUnit(item._id, e.target.value)
                      }
                    ...
                    <button
                      onClick={() =>
                        onUpdateQuantity(item._id, (item.quantity || 1) + 0.5)
                      }
                      className="w-6 h-6 rounded bg-rose/20 text-rose text-xs flex items-center justify-center hover:bg-rose/30 transition-colors"
                    >
                      <span className="material-symbols-outlined text-xs">
                  ...
                  <div className="p-3 sm:p-4">
                    ...
                    {item.price > 0 && (
                      <div className="flex items-baseline gap-1.5">
                        {hasDiscount ? (
                          <>
                            <span className="text-rose font-bold text-sm sm:text-base">
                              ₹{item.discountPrice}
                            </span>
                            <span className="text-cream/30 line-through text-[10px]">
                              ₹{item.price}
                            </span>
                          </>
                        ) : (
                          <span className="text-cream font-bold text-sm sm:text-base">
                            ₹{item.price}
                          </span>
                        )}
                        {item.priceUnit && (
...
export default function Menu({ isHomePage = false }) {
  ...
  const { cart, addToCart, removeFromCart, updateCartItem, setIsCartOpen } =
    useCart();
  ...
  const categorizedItems = useMemo(() => {
    ...
    let sortedCategories = Object.entries(groups).sort((a, b) => {
  ...
  const proceedToOrder = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("selectCakeForOrder", {
        detail: {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="absolute top-0 left-1/4 w-[300px] sm:w-[400px] md:w-[500px] h-[300px] sm:h-[400px] md:h-[500px] 
                      bg-rose/5 rounded-full blur-[80px] sm:blur-[100px] md:blur-[120px]"
        />
        <div
          className="absolute bottom-0 right-1/4 w-[200px] sm:w-[300px] md:w-[400px] h-[200px] sm:h-[300px] md:h-[400px] 
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-4 sm:px-6 md:px-8">
        ...
        >
          ...
          <p className="text-cream-muted text-sm sm:text-base md:text-lg px-4 sm:px-0 mb-4">
            Tap items to add to your order. Select multiple items and proceed
            when ready!
          </p>
        ...
        {isHomePage && (
          ...
          >
            <p className="text-cream-muted text-xs sm:text-sm mb-3 sm:mb-4 px-4 sm:px-0">
              Don&apos;t see your favorite flavor? We can create custom flavors
              too!
            </p>
      ...
      <AnimatePresence>
        {selectedItem && (
          <QuickViewModal
            ...
            isInOrder={cart.items.some(
              (o) => o.product._id === selectedItem._id,
            )}
            allItems={allFlavors}
            categoryItems={
              selectedItemCategory
                ? categorizedItems[selectedItemCategory] || []
                : []
            }
